...
const typeNum = (arr) => {
  let tempArr=[];
  tempArr=arr.filter(num=>typeof num==='number');
  return tempArr;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const containsAnd = (arr) => {
  let tempArr=[];
  let counter = 0;
  tempArr=arr.filter(str=>str.includes('and')
  );
  return tempArr;
};
...
const oddValues = (arr) => {
  let tempArr=[];
  tempArr=arr.filter(num => num%2 === 1);
  return tempArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let tempArr=[];
  tempArr=arr.filter(num=>!forbiddenValues.includes(num)
  );
  return tempArr;
};
...
const getStatName = (arr, minBaseStat) => {
  const getBaseStatGreaterThan = (arr, minBaseStat) => {
    let tempArr=[];
    tempArr=arr.filter(stat=>stat.baseStat>minBaseStat);
    return tempArr;
  }
...
const getCharactersWithoutChildren = (arr) => {
  let tempArr=[];
  tempArr=arr.filter(stat=>stat.baseStat>minBaseStat).map(stat=>stat.stat.name);
  return tempArr;
};
...
const evenOddNumericValues = (arr) => {
  ...
  tempArr=arr.filter(num=>typeof num==='number').map(num=>num%2===0? 'even' : 'odd');
  return tempArr;
};
