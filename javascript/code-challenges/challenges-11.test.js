...
function transformToLis(obj) {
  const newArr = Object.entries(obj).map(([key, value]) => { return `<li>${key}: ${value}</li>` });
  return newArr;
};
...
let findShortest = (data) => {
  let FinallValue = data.filter(character => character.height).reduce((a, b) => (b.height > a.height ? b : a));
  return FinallValue.name;
};
