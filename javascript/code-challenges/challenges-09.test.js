...
const getHouses = (arr) => {
  ...
  Object.values(arr).map(value => {
    return houses.push(value.house);
  });
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(value => {
    if (value.name === character) {
      hasChildren = value.children && value.children.length;
    }
  }
  );
  return hasChildren;
...
const hasChildrenEntries = (arr, character) => {
  return Object.entries(arr).find((obj) => {
    return obj[1].name === character && obj[1].children !== undefined;
  });
...
const houseSize = (arr) => {
  const sizes = [];
  for (let key of arr) {
    sizes.push({ house: key.house, members: Object.keys(key).length - 1 });
  }
  return sizes;
};
/* ------------------------------------------------------------------------------------------------
